...
Page.APIKeys = class APIKeys extends Page.Base {
	...
	onActivate(args) {
		...
		app.setHeaderTitle( '<i class="mdi mdi-key">&nbsp;</i>API Key Management' );
	...
	receive_keys(resp) {
		...
		html += this.getBasicTable( this.api_keys, cols, 'key', function(item, idx) {
			...
			return [
				'<div class="td_big">' + self.getNiceAPIKey(item, true) + '</div>',
		...
		html += '<div class="box_buttons">';
			html += '<div class="button primary" onMouseUp="$P().edit_api_key(-1)">Add API Key...</div>';
	...
	receive_key(resp) {
		...
		html += '<div class="box_buttons">';
			html += '<div class="button" onMouseUp="$P().cancel_api_key_edit()">Cancel</div>';
			html += '<div class="button" onMouseUp="$P().show_delete_api_key_dialog()">Delete Key...</div>';
	...
	show_delete_api_key_dialog() {
		...
		Dialog.confirm( '<span style="color:red">Delete API Key</span>', "Are you sure you want to <b>permanently delete</b> the API Key &ldquo;" + this.api_key.title + "&rdquo;?  There is no way to undo this action.", 'Delete', function(result) {
			if (result) {
	...
	get_api_key_edit_html() {
		...
		html += this.getFormRow({
			...
			caption: 'Select which privileges the API Key should have. Administrators have <b>all</b> privileges.'
		});
