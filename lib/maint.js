...
class Maintenance {
	...
	setupMaint() {
		...
		if (fs.existsSync(this.stats.file)) {
	...
	runDatabaseMaintenance(callback) {
		...
		this.storage.searchRecords( search_query, index_config, function(err, results) {
			...
			self.unbase.bulkDelete( 'messages', records, function(err) {
				...
				// also delete message headers
				// Note: Yes, we need to call Object.keys(results) again, because unbase.bulkDelete MUTILATES the array (sigh)
				async.eachLimit( Object.keys(results), self.storage.config.get('concurrency'),
					function(id, callback) {
						self.storage.delete( 'headers/' + id, function(err) {
							// ignore error here, as only some messages have headers
							callback();
						});
					},
					function() {
