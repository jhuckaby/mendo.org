...
class Mendo extends Component {
	...
	startup(callback) {
		...
		this.web.addURIHandler( /^\/$/, "Home", function(args, callback) {
			if (args.request.headers['user-agent'] && args.request.headers['user-agent'].match(/\bMSIE\b/)) {
				// legacy IE 6 thru 10 - unsupported
				args.internalFile = Path.resolve('htdocs/unsupported.html');
			}
			else if (args.request.headers['user-agent'] && args.request.headers['user-agent'].match(/\bTrident\b/)) {
				// IE 11 - supported via babel
				args.internalFile = Path.resolve('htdocs/ie.html');
			}
			else {
		...
		Object.keys(redirects).forEach( function(match) {
			self.web.addURIHandler( new RegExp(match), "Redirect", function(args, callback) {
				callback(  "302 Found", { 
					Location: self.web.getSelfURL(args.request, redirects[match]) 
				}, null );
			}); // addURIHandler
		}); // foreach redirect
	...
	beforeUserCreate(args, callback) {
		...
		user.searches = [
			{ "name": "My Posts", "query": "from:" + user.email + " type:topic" },
			{ "name": "My Replies", "query": "from:" + user.email + " type:reply" }
		];
