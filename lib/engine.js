...
class Mendo extends Component {
	...
	prewarm(callback) {
		// prewarm DB on startup
		var self = this;
		var dargs = Tools.getDateArgs( Tools.timeNow(true) );
		
		if (!this.config.get('prewarm_db')) return process.nextTick(callback);
		
		var queries = [
			{ query: 'type:topic date:' + dargs.yyyy + '/' + dargs.mm }, // recent
			{ query: 'type:topic date:' + dargs.yyyy + '/' + dargs.mm + ' replies:>0', sort_by: 'replies', sort_dir: -1 }, // hot
			{ query: 'type:topic tags:events when:' + dargs.yyyy + '/' + dargs.mm } // calendar
		];
		
		// add all category (tag) queries too
		this.tags.forEach( function(tag) {
			queries.push( { query: 'type:topic tags:' + tag.id } );
		} );
		...
		async.eachSeries( queries,
			function(query, callback) {
				query.offset = 0;
				query.limit = 50;
				if (!query.sort_by) query.sort_by = '_id';
				if (!query.sort_dir) query.sort_dir = -1;
				
				self.logDebug(5, "Prewarming DB/cache with common query", query);
				self.unbase.search( 'messages', query.query, query, function(err, results) {
					if (err) self.logError('unbase', "" + err);
					callback();
				});
			},
			callback 
	...
	beforeUserUpdate(args, callback) {
		...
		if (!args.admin_user && updates.email && (updates.email != user.email)) {
			user.verified = false;
		...
		// admin update with force-verify
		if (args.admin_user && !user.verified && updates.verified) {
			args.write_email_hash = updates.email || user.email;
		}
	...
	shutdown(callback) {
		...
		// (caused by some kind of bad HTTP request that I cannot track down)
