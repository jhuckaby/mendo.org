...
class FileManagement {
	...
	api_upload_files(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			if (session.type == 'user') {
				storage_key_prefix = 'files/' + user.username + '/' + Tools.generateShortID();
			}
			else {
				// API key
				storage_key_prefix = 'files/' + user.id + '/' + Tools.generateShortID();
				exp_epoch = Tools.timeNow(true) + Tools.getSecondsFromText("30 days");
			}
	...
	api_file(args, callback) {
		...
		this.logDebug(9, "Viewing file: " + storage_key);
