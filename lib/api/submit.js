...
class Submit {
	...
	finishNewTopic(message, user, callback) {
		...
		// FUTURE NOTE: For now, restrict this to the first date range found in the body text (slice 0,1)
		// Chrono tends to pick up very loose things as dates (e.g. "ages 6 years and over" --> 01-01-2026)
		
		try { 
			Chrono.parse(temp, ref, { forwardDate: true }).slice(0, 1).forEach( function(result) {
