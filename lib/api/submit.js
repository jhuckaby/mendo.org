...
class Submit {
	...
	api_update_post(args, callback) {
		// update topic or reply from UI
		...
		this.loadSession(args, function(err, session, user) {
			...
			self.unbase.get( 'messages', params.id, function(err, record) {
				if (err) return self.doError('post', "Message not found: " + err, callback);
				
				// make sure user is author
				if (record.username != user.username) {
					return self.doError('post', "Access Denied: You can only update your own posts", callback);
				}
				...
				self.unbase.update( 'messages', params.id, params, function(err) {
					...
					self.logUserActivity( user.username, 'message_update', self.getClientInfo(args, { 
						message: { type: record.type, id: params.id, subject: params.subject }
					}));
		...
		this.loadSession(args, function(err, session, user) {
			...
			self.unbase.get( 'messages', params.parent, function(err, parent_record) {
				...
				if (params.send_to_sender)  {
					...
					mail_text += 'From: ' + params.from + "\n";
					mail_text += 'Sender: ' + self.config.get('bounce_reply') + "\n";
					mail_text += 'Subject: Re: ' + parent_record.subject + "\n";
					mail_text += 'X-Mendo-Org: v' + self.server.__version + "\n";
					mail_text += 'Message-ID: ' + message.mid + "\n";
					if (parent_record.mid) {
			...
			self.loadUserFromEmail( message.from, function(user) {
				...
				self.lookupParentMessage( message, function(err, parent_id) {
					if (parent_id) {
						...
						if (!message.body.match(/\S/)) {
							// if body is empty, skip the planar stripped, and just go with full text
					...
					if (!message.body.match(/\S/)) {
						...
						if (message.body === '(No subject)') message.body = '(No content)';
					}
					...
					message.references = message.headers.references || '';
	...
	lookupParentMessage(message, callback) {
		...
		if (!message.headers.references && message.headers['in-reply-to']) {
			message.headers.references = message.headers['in-reply-to'];
		}
