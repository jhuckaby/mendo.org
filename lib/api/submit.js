...
class Submit {
	...
	lookupParentMessage(message, callback) {
		...
		if (message.headers.references) {
			var refs = message.headers.references.trim().split(/\s+/);
			var parent_id = '';
			...
			if (results.total) {
				for (var idx = 0, len = results.records.length; idx < len; idx++) {
					...
					if (test_subject == subject_clean) {
						parent_id = results.records[idx].id;
						idx = len;
					}
				} // foreach record
				
				if (!parent_id) {
					// still no match?  okay, if the most recent fuzzy match is within 24 hours, call it good enough
					// FUTURE: maybe strip commonly added words like "correction" and "update", and re-search?
					if (results.records[0].date >= message.date - 86400) {
						parent_id = results.records[0].id;
					}
				}
