...
class Submit {
	...
	api_external_view(args, callback) {
		// load single message externally without login
		// (for sharing thread URLs)
		// { id }
		var self = this;
		var params = Tools.mergeHashes( args.params, args.query );
	...
	api_doc(args, callback) {
		...
		if (params.record) {
			// view DB record as document
			args.params.id = params.record;
			return this.api_external_view(args, callback);
		}
