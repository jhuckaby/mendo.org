// Mendo.org Auto Installer
// Copyright (c) 2020 Joseph Huckaby, MIT License.
// https://github.com/jhuckaby/mendo.org
...
var base_dir = '/opt/mendo';
var log_dir = base_dir + '/logs';
var log_file = '';
var gh_repo_url = 'http://github.com/jhuckaby/mendo.org';
var gh_releases_url = 'https://api.github.com/repos/jhuckaby/mendo.org/releases';
var gh_head_tarball_url = 'https://github.com/jhuckaby/mendo.org/archive/master.tar.gz';
...
if (process.getuid() != 0) {
	die( "The Mendo.org auto-installer must be run as root." );
}
...
print( 
	"\n" + "Mendo.org Installer v" + installer_version + "\n" + 
	"Copyright (c) 2020 Mendo.org. MIT Licensed.\n" + 
	"Log File: " + log_file + "\n\n" 
);
...
if (is_preinstalled) {
	var pid_file = log_dir + '/mendo.pid';
	try {
...
cp.exec('curl -s ' + gh_releases_url, function (err, stdout, stderr) {
	...
	if (is_preinstalled) print("Upgrading Mendo.org from v"+cur_version+" to v"+new_version+"...\n");
	else print("Installing Mendo.org v"+new_version+"...\n");
	
	if (is_running) {
		print("\n");
		try { cp.execSync( base_dir + "/bin/control.sh stop", { stdio: 'inherit' } ); }
		catch (err) { die("Failed to stop Mendo.org: " + err); }
		print("\n");
	}
				...
				if (is_preinstalled) {
					...
					if (err) {
						...
						if (is_running) {
							...
							catch (err) { die("Failed to start Mendo.org: " + err); }
							print("\n");
						}
					}
				} // upgrade
